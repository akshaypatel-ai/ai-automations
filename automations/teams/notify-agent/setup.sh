#!/usr/bin/env bash
# Interactive installer for the Microsoft Teams Notify Agent.
...
say "Microsoft Teams Notify Agent — setup"
cat <<'EOF'
Installs an AI notifier into one of YOUR repositories:

  ship:     release published → AI-written plain-language announcement pushed to Teams
  incident: monitored workflow fails → calm what/impact/cause note to Teams
  ask:      (optional) @mention the agent → grounded answers from the repo

ship/incident need NO relay — a channel webhook URL is enough. Only `ask`
needs the small Cloudflare relay for Teams' signed outgoing webhook.

You'll need: a GitHub repo, a Teams channel webhook URL (channel → Workflows
→ "Post to a channel when a webhook request is received", or a classic
Incoming Webhook connector), and Claude auth.
EOF
...
AGENT_DIR="$TARGET/scripts/teams-agent"
...
WORKER_NAME="$(d worker_name "${repo_slug}-teams-relay")"
...
RENDER_VARS="GITHUB_REPO HANDLERS INCIDENT_WORKFLOWS AGENT_NAME PROJECT_NAME AUDIENCE STACK_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/teams-agent/ai/brain.sh  ($BRAIN_NAME)"
...
jq -n \
  ...
  '{recipe: "teams/notify-agent", runtime: "github-actions", brain: $brain,
...
echo "  + scripts/teams-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/teams-agent.yml" || true

say "Next steps (in order)"
cat <<EOF
1. GitHub secrets on $GITHUB_REPO:
     gh secret set TEAMS_WEBHOOK_URL -R $GITHUB_REPO        # the channel webhook URL
...
3. ONLY if you enabled 'ask' — deploy the relay and create the outgoing webhook:
   ...
   In Teams: Team → Manage team → Apps → Create an outgoing webhook
     Name: $AGENT_NAME · Callback URL: https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/
   Teams shows a security token EXACTLY ONCE — store it immediately:
     wrangler secret put TEAMS_SECURITY_TOKEN
   Use the outgoing webhook in the SAME channel the notify webhook posts to —
   answers arrive there.

4. Test without sending anything:
     cd $TARGET && DRY_RUN=1 MODE=ship RELEASE_TAG=v0.0.0 bash scripts/teams-agent/agent-run.sh
   Real test: Actions → Teams Notify Agent → Run workflow → mode: ship.

Operating docs: scripts/teams-agent/README.md
EOF
