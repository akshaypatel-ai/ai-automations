...
#   ask:      ASK_TEXT (the @mention question, mention already stripped)
# DRY_RUN=1 assembles the prompt but skips the AI and the push.
set -euo pipefail

cd "$(git rev-parse --show-toplevel)"
SCRIPTS="scripts/teams-agent"
...
case "$MODE" in
  ...
  incident)
    ...
    ;;
  ask)
    printf '## Question from Teams (@mention)\n\n%s\n' "${ASK_TEXT:-"(empty)"}" > "$CONTEXT_FILE"
    ;;
  *)
...
EOF
  ...
  echo
  echo "- mode: $MODE"
  echo "- push_helper (use this to send your ONE message): scripts/teams-agent/push.sh <body-file>"
