#!/usr/bin/env bash
# Interactive installer for the Basecamp Board Agent.
...
say "Basecamp Board Agent — setup"
cat <<'EOF'
...
It analyzes new cards, answers replies, and turns approved cards into
ready-for-review pull requests. You'll need: a GitHub repo, Basecamp access,
a free Cloudflare account, and a Claude subscription token or Anthropic API key.
...
say "Conventions"
ask AGENT_NAME "Agent display name (signs every comment)" "$(d agent_name 'Board Agent')"
AGENT_MARKER="🤖 $AGENT_NAME"
ask PROJECT_NAME "Product name used in comments" "$(d project_name "$(basename "$TARGET")")"
ask AUDIENCE "Who reads the board? (comments are written for them)" "$(d audience 'the team')"
...
cat <<EOF
  ...
  Basecamp           account $BC_ACCOUNT_ID · project $BC_PROJECT_ID · table $BC_CARD_TABLE_ID
...
RENDER_VARS="GITHUB_REPO BC_ACCOUNT_ID BC_PROJECT_ID BC_CARD_TABLE_ID BC_COL_ANALYZE BC_COL_IMPLEMENT COL_ANALYZE_NAME COL_IMPLEMENT_NAME AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE CLAUDE_MODEL WORKER_NAME"
...
jq -n \
  ...
  '{recipe: "basecamp/board-agent", runtime: "github-actions", brain: "claude-code",
    github_repo: $github_repo, bc_account_id: $bc_account_id, bc_project_id: $bc_project_id,
    bc_card_table_id: $bc_card_table_id, bc_col_analyze: $bc_col_analyze,
    col_analyze_name: $col_analyze_name, bc_col_implement: $bc_col_implement,
    col_implement_name: $col_implement_name, agent_name: $agent_name,
...
4. Register the Basecamp webhook (same secret in the URL path):
     basecamp webhooks create "https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook/<WEBHOOK_SECRET>" \\
       --types "Kanban::Card,Comment" --in $BC_PROJECT_ID
