...
# Usage: run-playbook.sh <analyze|respond|implement> <task_gid> [decision_json]
set -euo pipefail
...
- section_analyze_name: $SECTION_ANALYZE
- section_implement_name: $SECTION_IMPLEMENT
- agent_comment_marker (your comments MUST start with this, followed by " — "): $AGENT_MARKER
- api_helper (REST, responses wrap in .data): scripts/asana-agent/api.sh <METHOD> <path> [json-body]
- comment_helper (use this to post your ONE comment): scripts/asana-agent/comment.sh <task_gid> <body-file>
...
echo "running playbook '$PLAYBOOK' for task $ITEM_ID (brain: $AI_NAME, model: ${CLAUDE_MODEL:-default})"
