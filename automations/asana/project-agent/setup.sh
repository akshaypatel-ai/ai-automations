...
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/asana-agent"
...
jq -n \
  ...
  '{recipe: "asana/project-agent", runtime: $runtime, brain: $brain,
...
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/asana-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "ASANA_TOKEN"
