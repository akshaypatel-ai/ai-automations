#!/usr/bin/env bash
# Interactive installer for the Jira Project Agent.
...
say "Jira Project Agent — setup"
cat <<'EOF'
Installs an event-driven AI agent into one of YOUR repositories:

  Jira webhook (admin-registered, JQL-filtered) → Cloudflare Worker relay → GitHub Actions → headless Claude Code

Flow: issue enters your ANALYZE status → the agent posts options/questions;
discuss in comments; move it to your IMPLEMENT status → it builds the change
and opens a ready-for-review PR titled with the issue key. It never transitions
issues — humans own the board.

You'll need: a GitHub repo, a Jira Cloud site + API token
(id.atlassian.com → Security → API tokens), Jira ADMIN access to register the
webhook, a free Cloudflare account, and Claude auth.
EOF
...
AGENT_DIR="$TARGET/scripts/jira-agent"
...
say "Jira"
ask JIRA_SITE "Jira site URL (https://<yours>.atlassian.net)" "$(d jira_site '')"
JIRA_SITE="${JIRA_SITE%/}"
ask JIRA_PROJECT_KEY "Project key (the NOVA in NOVA-123)" "$(d jira_project_key '')"
ask STATUS_ANALYZE "Status the agent ANALYZES issues in" "$(d status_analyze 'To Do')"
ask STATUS_IMPLEMENT "Status the agent IMPLEMENTS issues from" "$(d status_implement 'In Progress')"
HANDLERS="issues"
...
WORKER_NAME="$(d worker_name "${repo_slug}-jira-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  Jira               $JIRA_SITE · project $JIRA_PROJECT_KEY
  ...
  Branches           $BRANCH_PREFIX/<key>-<slug> → PRs into $PR_BASE
...
RENDER_VARS="GITHUB_REPO HANDLERS JIRA_SITE JIRA_PROJECT_KEY STATUS_ANALYZE STATUS_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/jira-agent/state.sh  (core/state/git-branch.sh)"
mkdir -p "$AGENT_DIR/ai"
install -m 0644 "$BRAIN_FILE" "$AGENT_DIR/ai/brain.sh"
echo "  + scripts/jira-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/jira-agent"

jq -n \
  --arg runtime "$RUNTIME_NAME" \
  --arg github_repo "$GITHUB_REPO" --arg jira_site "$JIRA_SITE" \
  --arg jira_project_key "$JIRA_PROJECT_KEY" --arg status_analyze "$STATUS_ANALYZE" \
  ...
  '{recipe: "jira/project-agent", runtime: $runtime, brain: $brain,
    handlers: "issues", github_repo: $github_repo, jira_site: $jira_site,
    jira_project_key: $jira_project_key, status_analyze: $status_analyze,
...
echo "  + scripts/jira-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/jira-agent.yml" || true

say "GitHub secrets"
note "Required on $GITHUB_REPO:"
note "  JIRA_EMAIL + JIRA_API_TOKEN — id.atlassian.com → Security → API tokens"
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in JIRA_EMAIL JIRA_API_TOKEN $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
4. Register the webhook (needs a Jira ADMIN): $JIRA_SITE → Settings →
   System → WebHooks → Create:
     URL:    https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook/<WEBHOOK_SECRET>
...
6. Safe local test — no comments, no AI, no state pushed
   (export JIRA_EMAIL + JIRA_API_TOKEN first):
     cd $TARGET && DRY_RUN=1 bash scripts/jira-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/jira-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/jira-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "JIRA_EMAIL JIRA_API_TOKEN"
