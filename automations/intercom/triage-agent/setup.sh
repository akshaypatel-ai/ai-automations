#!/usr/bin/env bash
# Interactive installer for the Intercom Triage Agent.
...
say "Intercom Triage Agent — setup"
cat <<'EOF'
Installs an event-driven AI support-triage agent into one of YOUR repositories:

  Intercom webhook (signed) → Cloudflare Worker relay → GitHub Actions → headless Claude Code
...
It NEVER messages customers and never closes, assigns, or snoozes — humans
own the inbox.

You'll need: a GitHub repo, an Intercom app in the Developer Hub (access
token + client secret), a free Cloudflare account, and Claude auth.
EOF
...
AGENT_DIR="$TARGET/scripts/intercom-agent"
...
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/intercom-agent"
...
jq -n \
  ...
  '{recipe: "intercom/triage-agent", runtime: $runtime, brain: $brain,
...
3. Deploy the relay (free Cloudflare account; npm i -g wrangler):
     ...
     wrangler secret put INTERCOM_CLIENT_SECRET   # Developer Hub → your app → Basic information → Client secret

4. Point Intercom at the relay: Developer Hub
   (https://app.intercom.com/a/apps/_/developer-hub) → create or select your
   app → Webhooks → endpoint URL:
     https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook
   and subscribe to these topics:
     conversation.user.created
     conversation.user.replied
     conversation.admin.replied
   (Intercom probes the URL with a HEAD request on save — the relay answers
   it. INTERCOM_TOKEN comes from the SAME app: Authentication → Access token.)
...
6. Safe local test — no notes, no AI, no state pushed
   (export INTERCOM_TOKEN first):
     cd $TARGET && DRY_RUN=1 bash scripts/intercom-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/intercom-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/intercom-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "INTERCOM_TOKEN"
