...
RENDER_VARS="GITHUB_REPO HANDLERS AZDO_ORG AZDO_PROJECT STATE_ANALYZE STATE_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/azdo-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "azuredevops/project-agent", runtime: "github-actions", brain: "claude-code",
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
    for s in AZDO_PAT CLAUDE_CODE_OAUTH_TOKEN ANTHROPIC_API_KEY AGENT_GH_PAT; do
