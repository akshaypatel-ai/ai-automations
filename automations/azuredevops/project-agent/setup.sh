#!/usr/bin/env bash
# Interactive installer for the Azure DevOps Project Agent.
...
say "Azure DevOps Project Agent — setup"
cat <<'EOF'
Installs an event-driven AI agent into one of YOUR repositories:

  Azure DevOps service hooks (3 web-hook subscriptions) → Cloudflare Worker relay → GitHub Actions → headless Claude Code

Flow: work item enters your ANALYZE state → the agent posts options/questions;
discuss in comments; move it to your IMPLEMENT state → it builds the change
and opens a ready-for-review PR titled [AB-<id>]. It never changes work item
states — humans own the board.

You'll need: a GitHub repo, an Azure DevOps organization + project, a Personal
Access Token (dev.azure.com → User settings → Personal access tokens; scope
Work Items Read & Write), project-settings access to create the service hooks,
...
AGENT_DIR="$TARGET/scripts/azdo-agent"
...
say "Azure DevOps"
ask AZDO_ORG "Organization (the <org> in dev.azure.com/<org>)" "$(d azdo_org '')"
ask AZDO_PROJECT "Project name (as shown in Boards; spaces are fine)" "$(d azdo_project '')"
ask STATE_ANALYZE "State the agent ANALYZES work items in" "$(d state_analyze 'New')"
ask STATE_IMPLEMENT "State the agent IMPLEMENTS work items from" "$(d state_implement 'Active')"
HANDLERS="workitems"
...
ask AUDIENCE "Who reads these work items? (comments are written for them)" "$(d audience 'the team')"
...
WORKER_NAME="$(d worker_name "${repo_slug}-azdo-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  Azure DevOps       dev.azure.com/$AZDO_ORG · project $AZDO_PROJECT
  ...
  Branches           $BRANCH_PREFIX/ab-<id>-<slug> → PRs into $PR_BASE
...
RENDER_VARS="GITHUB_REPO HANDLERS AZDO_ORG AZDO_PROJECT STATE_ANALYZE STATE_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/azdo-agent/state.sh  (core/state/git-branch.sh)"
mkdir -p "$AGENT_DIR/ai"
install -m 0644 "$BRAIN_FILE" "$AGENT_DIR/ai/brain.sh"
echo "  + scripts/azdo-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/azdo-agent"

jq -n \
  --arg runtime "$RUNTIME_NAME" \
  --arg github_repo "$GITHUB_REPO" --arg azdo_org "$AZDO_ORG" \
  --arg azdo_project "$AZDO_PROJECT" --arg state_analyze "$STATE_ANALYZE" \
  ...
  '{recipe: "azuredevops/project-agent", runtime: $runtime, brain: $brain,
    handlers: "workitems", github_repo: $github_repo, azdo_org: $azdo_org,
    azdo_project: $azdo_project, state_analyze: $state_analyze,
...
echo "  + scripts/azdo-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/azdo-agent.yml" || true

say "GitHub secrets"
note "Required on $GITHUB_REPO:"
note "  AZDO_PAT — dev.azure.com → User settings → Personal access tokens (scope Work Items Read & Write)"
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in AZDO_PAT $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
4. Create THREE service-hook subscriptions (dev.azure.com/$AZDO_ORG →
   $AZDO_PROJECT → Project settings → Service hooks → + → Web Hooks),
   all pointing at:
     https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook/<WEBHOOK_SECRET>
     a) Trigger: Work item created
     b) Trigger: Work item updated — Field: State (server-side noise filter)
     c) Trigger: Work item commented on
   Azure DevOps doesn't sign deliveries — the secret in the URL is the
   authentication (same model as the Jira recipe). The form's optional
   Basic-auth username/password fields are extra hardening if you want them;
   the relay doesn't require it.
...
6. Safe local test — no comments, no AI, no state pushed
   (export AZDO_PAT first):
     cd $TARGET && DRY_RUN=1 bash scripts/azdo-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/azdo-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/azdo-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "AZDO_PAT"
