...
#   ask:      ASK_TEXT, ASK_CHAT_ID (defaults to TELEGRAM_CHAT_ID)
# DRY_RUN=1 assembles the prompt but skips the AI and the push.
set -euo pipefail

cd "$(git rev-parse --show-toplevel)"
SCRIPTS="scripts/telegram-agent"
...
case "$MODE" in
  ...
  incident)
    ...
    ;;
  ask)
    printf '## Question from the Telegram chat\n\n%s\n' "${ASK_TEXT:-"(empty)"}" > "$CONTEXT_FILE"
    ;;
  *)
...
CHAT_ID="${ASK_CHAT_ID:-$TELEGRAM_CHAT_ID}"
...
EOF
  ...
  echo
  echo "- mode: $MODE"
  echo "- chat_id (push here): $CHAT_ID"
  echo "- push_helper (use this to send your ONE message): scripts/telegram-agent/push.sh <chat_id> <body-file>"
...
if [[ "${CAN_RUN_TOOLS:-1}" == "0" ]]; then
  ...
  printf '%s\n' "$body" > "$body_file"
  "$SCRIPTS/push.sh" "$CHAT_ID" "$body_file"
  case "$MODE" in
