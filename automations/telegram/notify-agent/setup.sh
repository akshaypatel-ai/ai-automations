#!/usr/bin/env bash
# Interactive installer for the Telegram Notify Agent.
...
say "Telegram Notify Agent — setup"
cat <<'EOF'
Installs an AI notifier into one of YOUR repositories:

  ship:     release published → AI-written plain-language announcement pushed to Telegram
  incident: monitored workflow fails → calm what/impact/cause note to Telegram
  ask:      (optional) chat members message the bot → grounded answers from the repo

ship/incident need NO relay — they are GitHub-native triggers. Only `ask`
needs the small Cloudflare relay for Telegram's webhook.

You'll need: a GitHub repo, a Telegram bot (@BotFather → /newbot; the token
is the CI secret), the bot added to your group/channel, and Claude auth.
For the chat id, add @RawDataBot or @userinfobot to the group and read it
from their reply — group ids are negative numbers; a DM uses your user id.
EOF
...
AGENT_DIR="$TARGET/scripts/telegram-agent"
...
say "Telegram"
ask TELEGRAM_CHAT_ID "Chat id to push to (group/channel/user; groups are negative)" "$(d telegram_chat_id '')"
...
ask AUDIENCE "Who reads the Telegram chat? (messages are written for them)" "$(d audience 'the team')"
...
WORKER_NAME="$(d worker_name "${repo_slug}-telegram-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  Push chat          $TELEGRAM_CHAT_ID
  Handlers           $HANDLERS
...
RENDER_VARS="GITHUB_REPO HANDLERS TELEGRAM_CHAT_ID INCIDENT_WORKFLOWS AGENT_NAME PROJECT_NAME AUDIENCE STACK_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/telegram-agent/ai/brain.sh  (core/ai/claude-code.sh)"
chmod +x "$AGENT_DIR"/*.sh

jq -n \
  --arg github_repo "$GITHUB_REPO" --arg handlers "$HANDLERS" \
  --arg telegram_chat_id "$TELEGRAM_CHAT_ID" --arg wf_names "$WF_NAMES" \
  ...
  '{recipe: "telegram/notify-agent", runtime: "github-actions", brain: "claude-code",
    handlers: $handlers, github_repo: $github_repo, telegram_chat_id: $telegram_chat_id,
...
echo "  + scripts/telegram-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/telegram-agent.yml" || true

say "Next steps (in order)"
cat <<EOF
1. GitHub secrets on $GITHUB_REPO:
     gh secret set TELEGRAM_BOT_TOKEN -R $GITHUB_REPO       # from @BotFather (/newbot or /token)
...
3. ONLY if you enabled 'ask' — deploy the relay and register the webhook:
     cd $AGENT_DIR/relay
     wrangler deploy
     wrangler secret put GITHUB_PAT               # fine-grained PAT: only $GITHUB_REPO, contents: read+write
     wrangler secret put TELEGRAM_WEBHOOK_SECRET  # e.g. openssl rand -hex 24
   Then tell Telegram where to deliver (same secret as above):
     curl "https://api.telegram.org/bot<TOKEN>/setWebhook" \\
       -d "url=https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook" \\
       -d "secret_token=<TELEGRAM_WEBHOOK_SECRET>" \\
       -d 'allowed_updates=["message","channel_post"]'
   Group privacy mode: by default the bot only receives messages that mention
   it or reply to it — fine for Q&A. To let it see every group message:
   @BotFather → /setprivacy → Disabled (re-add the bot to the group after).

4. Test without sending anything:
     cd $TARGET && DRY_RUN=1 MODE=ship RELEASE_TAG=v0.0.0 bash scripts/telegram-agent/agent-run.sh
   Real test: Actions → Telegram Notify Agent → Run workflow → mode: ship.

Operating docs: scripts/telegram-agent/README.md
EOF
