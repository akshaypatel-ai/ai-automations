...
#   ask:      ASK_TEXT, ASK_APP_ID, ASK_TOKEN (interaction follow-up credentials)
# DRY_RUN=1 assembles the prompt but skips the AI and the push.
set -euo pipefail

cd "$(git rev-parse --show-toplevel)"
SCRIPTS="scripts/discord-agent"
...
case "$MODE" in
  ...
  incident)
    ...
    ;;
  ask)
    printf '## Question from Discord (/ask)\n\n%s\n' "${ASK_TEXT:-"(empty)"}" > "$CONTEXT_FILE"
    ;;
  *)
...
EOF
  ...
  echo "- mode: $MODE"
  if [[ "$MODE" == "ask" ]]; then
    echo "- ask_app_id: ${ASK_APP_ID:-}"
    echo "- ask_interaction_token: ${ASK_TOKEN:-}"
    echo "- answer_helper (use this to send your ONE answer): scripts/discord-agent/followup.sh <ask_app_id> <ask_interaction_token> <body-file>"
  else
    echo "- push_helper (use this to send your ONE message): scripts/discord-agent/push.sh <body-file>"
  fi
...
if [[ "${CAN_RUN_TOOLS:-1}" == "0" ]]; then
  ...
  printf '%s\n' "$body" > "$body_file"
  if [[ "$MODE" == "ask" ]]; then
    "$SCRIPTS/followup.sh" "$ASK_APP_ID" "$ASK_TOKEN" "$body_file"
  else
