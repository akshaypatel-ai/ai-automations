...
RENDER_VARS="GITHUB_REPO HANDLERS BUILDKITE_ORG BUILDKITE_PIPELINE ISSUE_LABEL AGENT_NAME AGENT_MARKER PROJECT_NAME STACK_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/buildkite-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "buildkite/build-agent", runtime: "github-actions", brain: "claude-code",
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
    for s in BUILDKITE_TOKEN CLAUDE_CODE_OAUTH_TOKEN ANTHROPIC_API_KEY AGENT_GH_PAT; do
