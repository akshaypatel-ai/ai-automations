...
# Usage: run-playbook.sh <analyze> <build_number> [decision_json]
set -euo pipefail
...
- build_number: $ITEM_ID
- build_url: $(jq -r '.web_url // ""' <<<"$DECISION_JSON")
...
- api_helper (REST, read-only toward Buildkite): scripts/buildkite-agent/api.sh <METHOD> <path> [json-body]
