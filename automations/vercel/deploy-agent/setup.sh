...
RENDER_VARS="GITHUB_REPO HANDLERS VERCEL_TEAM_ID VERCEL_PROJECT_ID ISSUE_LABEL AGENT_NAME AGENT_MARKER PROJECT_NAME STACK_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/vercel-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "vercel/deploy-agent", runtime: "github-actions", brain: "claude-code",
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
    for s in VERCEL_TOKEN CLAUDE_CODE_OAUTH_TOKEN ANTHROPIC_API_KEY AGENT_GH_PAT; do
