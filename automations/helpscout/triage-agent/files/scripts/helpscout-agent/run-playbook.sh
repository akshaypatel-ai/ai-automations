...
- api_helper (REST — it manages the OAuth token itself, you never handle auth; paths start with /v2): scripts/helpscout-agent/api.sh <METHOD> <path> [json-body]
- note_helper (use this to post your ONE internal note — write PLAIN TEXT, it converts to the HTML Help Scout expects, and it is structurally internal): scripts/helpscout-agent/note.sh <conversation_id> <body-file>
...
echo "running playbook '$PLAYBOOK' for conversation $ITEM_ID (brain: $AI_NAME, model: ${CLAUDE_MODEL:-default})"
