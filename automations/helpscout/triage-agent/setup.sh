#!/usr/bin/env bash
# Interactive installer for the Help Scout Triage Agent.
...
say "Help Scout Triage Agent — setup"
cat <<'EOF'
Installs an event-driven AI support-triage agent into one of YOUR repositories:

  Help Scout webhook (signed) → Cloudflare Worker relay → GitHub Actions → headless Claude Code

Flow: a customer conversation arrives → the agent grounds itself in your
product's repo and posts ONE internal note (category + a draft reply a human
can send nearly verbatim); real product bugs get escalated as GitHub issues.
It NEVER messages customers and never changes status, assignee, or tags —
humans own the queue.

You'll need: a GitHub repo, a Help Scout app (My Apps → Create App gives the
app id + secret), a free Cloudflare account, and Claude auth.
EOF
...
AGENT_DIR="$TARGET/scripts/helpscout-agent"
...
say "Help Scout"
ask_opt HELPSCOUT_MAILBOX_ID "Mailbox id to watch (the number in the mailbox URL; empty = all mailboxes)" "$(d helpscout_mailbox_id '')"
...
WORKER_NAME="$(d worker_name "${repo_slug}-helpscout-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  Mailbox            ${HELPSCOUT_MAILBOX_ID:-all mailboxes}
...
RENDER_VARS="GITHUB_REPO HANDLERS HELPSCOUT_MAILBOX_ID ESCALATION_LABEL AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/helpscout-agent/state.sh  (core/state/git-branch.sh)"
mkdir -p "$AGENT_DIR/ai"
install -m 0644 "$BRAIN_FILE" "$AGENT_DIR/ai/brain.sh"
echo "  + scripts/helpscout-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/helpscout-agent"

jq -n \
  --arg runtime "$RUNTIME_NAME" \
  --arg github_repo "$GITHUB_REPO" --arg helpscout_mailbox_id "$HELPSCOUT_MAILBOX_ID" \
  ...
  '{recipe: "helpscout/triage-agent", runtime: $runtime, brain: $brain,
    handlers: "conversations", github_repo: $github_repo,
    helpscout_mailbox_id: $helpscout_mailbox_id, escalation_label: $escalation_label,
...
echo "  + scripts/helpscout-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/helpscout-agent.yml" || true

say "GitHub secrets"
note "Required on $GITHUB_REPO:"
note "  HELPSCOUT_APP_ID + HELPSCOUT_APP_SECRET — Help Scout → your avatar → My Apps → Create App"
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in HELPSCOUT_APP_ID HELPSCOUT_APP_SECRET $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
3. Deploy the relay (free Cloudflare account; npm i -g wrangler):
     ...
     wrangler secret put HELPSCOUT_SECRET_KEY   # YOU choose this value (e.g. openssl rand -hex 24) — keep it for step 4

4. Create the webhook: Help Scout → Manage → Apps → Webhooks, with
     Callback URL: https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook
     Secret Key:   the SAME value you gave HELPSCOUT_SECRET_KEY in step 3
   and select exactly these events:
     Conversation Created            (convo.created)
     Customer Replied                (convo.customer.reply.created)
     Agent Replied                   (convo.agent.reply.created)
...
6. Safe local test — no notes, no AI, no state pushed
   (export HELPSCOUT_APP_ID and HELPSCOUT_APP_SECRET first):
     cd $TARGET && DRY_RUN=1 bash scripts/helpscout-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/helpscout-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/helpscout-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "HELPSCOUT_APP_ID HELPSCOUT_APP_SECRET"
