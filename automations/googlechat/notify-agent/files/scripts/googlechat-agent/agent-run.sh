...
# Env: MODE (ship|incident), plus per-mode context:
#   ship:     RELEASE_TAG
...
SCRIPTS="scripts/googlechat-agent"
source "$SCRIPTS/env.sh"
source "$SCRIPTS/ai/brain.sh"
mkdir -p "$OUT_DIR"

MODE="${MODE:?set MODE=ship|incident}"
...
prompt_file="$OUT_DIR/prompt-$MODE.md"
{
  ...
  echo
  echo "- mode: $MODE"
  echo "- push_helper (use this to send your ONE message): scripts/googlechat-agent/push.sh <body-file>"
