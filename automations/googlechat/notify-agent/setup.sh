...
RENDER_VARS="GITHUB_REPO HANDLERS INCIDENT_WORKFLOWS AGENT_NAME PROJECT_NAME AUDIENCE STACK_NOTE CLAUDE_MODEL"
...
echo "  + scripts/googlechat-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "googlechat/notify-agent", runtime: "github-actions", brain: "claude-code",
    ...
    stack_note: $stack_note, claude_model: $claude_model}' > "$CONFIG"
