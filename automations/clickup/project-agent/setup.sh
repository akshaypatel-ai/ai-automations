#!/usr/bin/env bash
# Interactive installer for the ClickUp Project Agent.
...
say "ClickUp Project Agent — setup"
cat <<'EOF'
Installs an event-driven AI agent into one of YOUR repositories:

  ClickUp webhook (HMAC-verified) → Cloudflare Worker relay → GitHub Actions → headless Claude Code

Flow: task enters your ANALYZE status → the agent posts options/questions;
discuss in comments; move it to your IMPLEMENT status → it builds the change
and opens a ready-for-review PR. It never changes task statuses — humans own
the board.

You'll need: a GitHub repo, a ClickUp API token (Settings → Apps → API token),
a free Cloudflare account, and Claude auth.

Finding IDs: the list id is in the URL when a list is open
(app.clickup.com/<team_id>/v/li/<list_id>); the team (workspace) id is the
number right after app.clickup.com/.
EOF
...
AGENT_DIR="$TARGET/scripts/clickup-agent"
...
say "ClickUp"
ask CLICKUP_TEAM_ID "Team (workspace) ID" "$(d clickup_team_id '')"
ask CLICKUP_LIST_ID "List ID to watch" "$(d clickup_list_id '')"
ask STATUS_ANALYZE "Status the agent ANALYZES tasks in" "$(d status_analyze 'to do')"
ask STATUS_IMPLEMENT "Status the agent IMPLEMENTS tasks from" "$(d status_implement 'in progress')"
HANDLERS="tasks"
...
WORKER_NAME="$(d worker_name "${repo_slug}-clickup-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  ClickUp            team $CLICKUP_TEAM_ID · list $CLICKUP_LIST_ID
...
RENDER_VARS="GITHUB_REPO HANDLERS CLICKUP_TEAM_ID CLICKUP_LIST_ID STATUS_ANALYZE STATUS_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/clickup-agent/state.sh  (core/state/git-branch.sh)"
mkdir -p "$AGENT_DIR/ai"
install -m 0644 "$BRAIN_FILE" "$AGENT_DIR/ai/brain.sh"
echo "  + scripts/clickup-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/clickup-agent"

jq -n \
  --arg runtime "$RUNTIME_NAME" \
  --arg github_repo "$GITHUB_REPO" --arg clickup_team_id "$CLICKUP_TEAM_ID" \
  --arg clickup_list_id "$CLICKUP_LIST_ID" --arg status_analyze "$STATUS_ANALYZE" \
  ...
  '{recipe: "clickup/project-agent", runtime: $runtime, brain: $brain,
    handlers: "tasks", github_repo: $github_repo, clickup_team_id: $clickup_team_id,
    clickup_list_id: $clickup_list_id, status_analyze: $status_analyze,
...
echo "  + scripts/clickup-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/clickup-agent.yml" || true

say "GitHub secrets"
note "Required on $GITHUB_REPO:"
note "  CLICKUP_TOKEN — ClickUp → Settings → Apps → API token"
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in CLICKUP_TOKEN $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
4. Create the webhook (returns a secret — put it into the relay right after):
     curl -X POST "https://api.clickup.com/api/v2/team/$CLICKUP_TEAM_ID/webhook" \\
       ...
       -d '{"endpoint": "https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook",
            "events": ["taskCreated","taskStatusUpdated","taskCommentPosted","taskMoved"]}'
     # copy .webhook.secret from the response, then:
...
6. Safe local test — no comments, no AI, no state pushed
   (export CLICKUP_TOKEN first):
     cd $TARGET && DRY_RUN=1 bash scripts/clickup-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/clickup-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/clickup-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "CLICKUP_TOKEN"
