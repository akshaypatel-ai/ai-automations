...
SCRIPTS="scripts/mattermost-agent"
...
case "$MODE" in
  ...
  incident)
    ...
    ;;
  ask)
    printf '## Question from Mattermost (trigger word)\n\n%s\n' "${ASK_TEXT:-"(empty)"}" > "$CONTEXT_FILE"
    ;;
  *)
...
prompt_file="$OUT_DIR/prompt-$MODE.md"
{
  ...
  echo
  echo "- mode: $MODE"
  echo "- push_helper (use this to send your ONE message): scripts/mattermost-agent/push.sh <body-file>"
