#!/usr/bin/env bash
# Interactive installer for the Rocket.Chat Notify Agent.
...
say "Rocket.Chat Notify Agent — setup"
cat <<'EOF'
Installs an AI notifier into one of YOUR repositories:

  ship:     release published → AI-written plain-language announcement pushed to Rocket.Chat
  incident: monitored workflow fails → calm what/impact/cause note to Rocket.Chat
...
needs the small Cloudflare relay for Rocket.Chat's outgoing webhook.

You'll need: a GitHub repo, a Rocket.Chat incoming webhook URL (Admin →
Integrations → Incoming WebHook — the URL embeds the token, it IS the
credential), and Claude auth.
EOF
...
AGENT_DIR="$TARGET/scripts/rocketchat-agent"
...
WORKER_NAME="$(d worker_name "${repo_slug}-rocketchat-relay")"
...
RENDER_VARS="GITHUB_REPO HANDLERS INCIDENT_WORKFLOWS TRIGGER AGENT_NAME PROJECT_NAME AUDIENCE STACK_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/rocketchat-agent/ai/brain.sh  ($BRAIN_NAME)"
...
jq -n \
  ...
  '{recipe: "rocketchat/notify-agent", runtime: "github-actions", brain: $brain,
...
echo "  + scripts/rocketchat-agent/automation.config.json"

render_check "$AGENT_DIR" "$TARGET/.github/workflows/rocketchat-agent.yml" || true

say "Next steps (in order)"
cat <<EOF
1. GitHub secrets on $GITHUB_REPO:
     gh secret set ROCKETCHAT_WEBHOOK_URL -R $GITHUB_REPO   # incoming webhook URL (Admin → Integrations → Incoming WebHook — the URL embeds the token, it IS the credential)
...
3. ONLY if you enabled 'ask' — deploy the relay and create the outgoing webhook:
   ...
   In Rocket.Chat: Admin → Integrations → New → Outgoing WebHook
     Event trigger: Message Sent
     Channel: the SAME channel the incoming webhook posts to (answers arrive there)
     Trigger words: $TRIGGER
     URLs: https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook
     Token: set one yourself (any long random string) — then store it:
     wrangler secret put ROCKETCHAT_OUTGOING_TOKEN
   Note: if answers re-trigger the outgoing webhook on your server config,
   the relay's bot-flag and marker guards absorb it — no reply loop.

4. Test without sending anything:
     cd $TARGET && DRY_RUN=1 MODE=ship RELEASE_TAG=v0.0.0 bash scripts/rocketchat-agent/agent-run.sh
   Real test: Actions → Rocket.Chat Notify Agent → Run workflow → mode: ship.

Operating docs: scripts/rocketchat-agent/README.md
EOF
