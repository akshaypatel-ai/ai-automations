...
RENDER_VARS="GITHUB_REPO HANDLERS TRELLO_BOARD_ID LIST_ANALYZE_ID LIST_ANALYZE_NAME LIST_IMPLEMENT_ID LIST_IMPLEMENT_NAME AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/trello-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/trello-agent"
...
jq -n \
  ...
  '{recipe: "trello/project-agent", runtime: $runtime, brain: $brain,
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in TRELLO_KEY TRELLO_TOKEN $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/trello-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "TRELLO_KEY TRELLO_TOKEN"
