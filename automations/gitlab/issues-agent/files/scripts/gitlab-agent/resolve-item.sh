#!/usr/bin/env bash
# Decide which playbook (if any) applies to a GitLab issue by diffing the API
# against saved state. Fully implemented: issues (label-driven — GitLab boards
# are label filters, so labels ARE the columns). Other item types skip cleanly.
# Usage: resolve-item.sh <issue_iid> [item_type]   → prints a one-line decision JSON.
set -euo pipefail
...
last_note_id=$(jq -r '.last_note_id // 0' <<<"$state")
phase=$(jq -r '.phase // "new"' <<<"$state")

issue_json=$("$SCRIPT_DIR/api.sh" GET \
  "/projects/$GITLAB_PROJECT_ID/issues/$ITEM_ID" 2>/dev/null) || skip "issue not fetchable"

[[ "$(jq -r '.state // ""' <<<"$issue_json")" == "opened" ]] || skip "issue not open"
...
# Note recency is tracked by the numeric note id (monotonic, like Jira's).
# System notes (label changes, milestone edits, …) are GitLab's own activity
# log — never human input, so they are excluded here.
notes=$("$SCRIPT_DIR/api.sh" GET \
  "/projects/$GITLAB_PROJECT_ID/issues/$ITEM_ID/notes?per_page=100&sort=asc" 2>/dev/null || echo '[]')
latest_note_id=$(jq -r '[.[].id] | max // 0' <<<"$notes")
new_human_count=$(jq --argjson last "$last_note_id" --arg marker "$AGENT_MARKER" \
  '[.[] | select(.id > $last)
       | select(.system != true)
...
jq -cn \
  ...
  --argjson latest_note_id "$latest_note_id" \
  ...
  '{item_id: $item_id, item_type: "Issue", decision: $decision, reason: $reason,
    ...
    prev_phase: $prev_phase, latest_note_id: $latest_note_id,
