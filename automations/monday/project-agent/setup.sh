...
RENDER_VARS="GITHUB_REPO HANDLERS MONDAY_BOARD_ID STATUS_COLUMN_ID STATUS_ANALYZE STATUS_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/monday-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "monday/project-agent", runtime: "github-actions", brain: "claude-code",
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
    for s in MONDAY_TOKEN CLAUDE_CODE_OAUTH_TOKEN ANTHROPIC_API_KEY AGENT_GH_PAT; do
