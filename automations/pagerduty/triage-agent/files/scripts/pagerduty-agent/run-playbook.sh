...
# Usage: run-playbook.sh <analyze> <incident_id> [decision_json]
set -euo pipefail
...
prompt_file="$OUT_DIR/prompt-$ITEM_ID-$PLAYBOOK.md"
{
  ...
  if [[ "${CAN_RUN_TOOLS:-1}" == "0" ]]; then
    ...
    echo
    echo "### Incident"
    fetch_capped "/incidents/$ITEM_ID"
    echo
    echo "### Alerts"
    fetch_capped "/incidents/$ITEM_ID/alerts"
    echo
    echo "### Notes"
    fetch_capped "/incidents/$ITEM_ID/notes"
    cat <<'EOF'
...
- incident_id: $ITEM_ID
- agent_marker (your note's first line starts with this): $AGENT_MARKER
- api_helper (REST — GET only beyond the note helper): scripts/pagerduty-agent/api.sh <METHOD> <path> [json-body]
- note_helper (posts your ONE note — plain text, adds the From header itself): scripts/pagerduty-agent/note.sh <incident_id> <body-file>
...
echo "running playbook '$PLAYBOOK' for incident $ITEM_ID (brain: $AI_NAME, model: ${AI_MODEL:-default})"
