...
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/pagerduty-agent"
...
jq -n \
  ...
  '{recipe: "pagerduty/triage-agent", runtime: $runtime, brain: $brain,
...
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/pagerduty-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "PAGERDUTY_TOKEN"
