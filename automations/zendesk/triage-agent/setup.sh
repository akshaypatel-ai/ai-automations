...
RENDER_VARS="GITHUB_REPO HANDLERS ZENDESK_SUBDOMAIN ESCALATION_LABEL AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE CLAUDE_MODEL WORKER_NAME"
...
echo "  + scripts/zendesk-agent/ai/brain.sh  (core/ai/claude-code.sh)"
...
jq -n \
  ...
  '{recipe: "zendesk/triage-agent", runtime: "github-actions", brain: "claude-code",
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
    for s in ZENDESK_EMAIL ZENDESK_API_TOKEN CLAUDE_CODE_OAUTH_TOKEN ANTHROPIC_API_KEY AGENT_GH_PAT; do
