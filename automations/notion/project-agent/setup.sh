...
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/notion-agent"
...
jq -n \
  ...
  '{recipe: "notion/project-agent", runtime: $runtime, brain: $brain,
...
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/notion-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "NOTION_TOKEN"
