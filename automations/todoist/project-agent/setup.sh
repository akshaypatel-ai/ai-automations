#!/usr/bin/env bash
# Interactive installer for the Todoist Project Agent.
...
say "Todoist Project Agent — setup"
cat <<'EOF'
Installs an event-driven AI agent into one of YOUR repositories:

  Todoist app webhook (base64 HMAC) → Cloudflare Worker relay → GitHub Actions → headless Claude Code

Flow: a task moves into your ANALYZE section → the agent posts options/questions;
...
You'll need: a GitHub repo, a Todoist API token
(Settings → Integrations → Developer → API token), a free Cloudflare account,
and Claude auth.

Finding the project id: it's in the project URL when the project is open
(app.todoist.com/app/project/<id> — v2 ids are alphanumeric; paste as-is).
EOF
...
AGENT_DIR="$TARGET/scripts/todoist-agent"
...
say "Todoist"
ask TODOIST_PROJECT_ID "Project id to watch" "$(d todoist_project_id '')"
...
WORKER_NAME="$(d worker_name "${repo_slug}-todoist-relay")"

say "Summary"
cat <<EOF
  Target repo        $TARGET
  GitHub repo        $GITHUB_REPO
  Todoist project    $TODOIST_PROJECT_ID
...
RENDER_VARS="GITHUB_REPO HANDLERS TODOIST_PROJECT_ID SECTION_ANALYZE SECTION_IMPLEMENT AGENT_NAME AGENT_MARKER PROJECT_NAME AUDIENCE STACK_NOTE BRANCH_PREFIX PR_BASE QA_NOTE BRAIN_NAME AI_MODEL WORKER_NAME"
...
echo "  + scripts/todoist-agent/state.sh  (core/state/git-branch.sh)"
mkdir -p "$AGENT_DIR/ai"
install -m 0644 "$BRAIN_FILE" "$AGENT_DIR/ai/brain.sh"
echo "  + scripts/todoist-agent/ai/brain.sh  ($BRAIN_NAME)"
chmod +x "$AGENT_DIR"/*.sh
runtime_render_ci "$RUNTIME_NAME" "$ROOT/core/runtimes" "$AGENT_DIR" "scripts/todoist-agent"

jq -n \
  --arg runtime "$RUNTIME_NAME" \
  --arg github_repo "$GITHUB_REPO" --arg todoist_project_id "$TODOIST_PROJECT_ID" \
  ...
  '{recipe: "todoist/project-agent", runtime: $runtime, brain: $brain,
    handlers: "tasks", github_repo: $github_repo, todoist_project_id: $todoist_project_id,
...
echo "  + scripts/todoist-agent/automation.config.json"

render_check "$AGENT_DIR" || true

say "GitHub secrets"
note "Required on $GITHUB_REPO:"
note "  TODOIST_TOKEN — Todoist → Settings → Integrations → Developer → API token"
...
if [[ "$RUNTIME_NAME" == "github-actions" ]]; then
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if confirm "Set secrets on $GITHUB_REPO now with gh?"; then
      for s in TODOIST_TOKEN $BRAIN_AUTH_VARS AGENT_GH_PAT; do
...
4. Create the Todoist app + webhook (developer.todoist.com/appconsole.html):
     - Create app → set Webhook callback URL to
       https://$WORKER_NAME.<your-cf-subdomain>.workers.dev/hook
     - Select events: item:added, item:updated, item:uncompleted, note:added → save
     - Store the app's Client Secret in the relay:
       cd $AGENT_DIR/relay && wrangler secret put TODOIST_CLIENT_SECRET
     - Activate deliveries: Todoist webhooks only fire for accounts that
       authorized the app — complete the app's OAuth flow ONCE with your own
       account (open its OAuth authorization link from the console and
       approve). Until then, webhooks stay silent.
...
6. Safe local test — no comments, no AI, no state pushed
   (export TODOIST_TOKEN first):
     cd $TARGET && DRY_RUN=1 bash scripts/todoist-agent/agent-run.sh

Operating docs (recovery, pausing, transcripts): scripts/todoist-agent/README.md
EOF
runtime_overlay "$RUNTIME_NAME" "scripts/todoist-agent" "$WORKER_NAME" "$BRAIN_AUTH_VARS" "TODOIST_TOKEN"
